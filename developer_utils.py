...
def reload_addon_modules(path, package_name, reload):
    """
...
def setup_addon_modules(package_name, modules):
    for module in modules:
        if 'setup' in dir(module):
            setup_func = getattr(module, 'setup')
            setup_func()

def teardown_addon_modules(package_name, modules):
    for module in modules:
        if 'teardown' in dir(module):
            setup_func = getattr(module, 'setup')
            setup_func()
